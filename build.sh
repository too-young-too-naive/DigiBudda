...
# ──────────────────────────────────────────────
SOUND_FOUND=false
for ext in mp3 wav m4a aiff caf; do
    [[ -f "${SRC_DIR}/Resources/woodenfish.${ext}" ]] && SOUND_FOUND=true && break
done
if [[ "$SOUND_FOUND" == "false" ]]; then
    echo "→ Generating placeholder wooden fish sound…"
    python3 scripts/generate_sound.py "${SRC_DIR}/Resources/woodenfish.wav"
fi
